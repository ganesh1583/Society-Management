...
<div class="table-container">
    <div class="table-header">Add New Amenity</div>
    <div class="table-content">
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
            Enter Amenity Name : <input type="text" name="amenity_name" required>
            <br> <br>
            ...
            <input type="radio" name="status" value="under-maintenance" required>Under Maintenance
            <br> <br>
        ...
        <?php
            if($_SERVER['REQUEST_METHOD'] == "POST")
            {
              ...
              $qry = "insert into amenities (society_id,name,description,category,status,created_at,updated_at) values ($society_id,'$amenity_name','$description','$category','$status',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)";

              $rs = pg_query($con,$qry);

              if(!$rs) 
              {
                  echo "<script> alert('Error while inserting amenity into amenity table '); </script>";
                  header("refresh:0;url=/Society_Management/dashboard/add_amenity.php");
              }
              
          
              echo '<script>alert("Success... Amenity Uploaded!");</script>';
              echo '<script>window.location.href = "/Society_Management/dashboard/amenities.php";</script>';
            }
        ?>
    </div>
  </div>
