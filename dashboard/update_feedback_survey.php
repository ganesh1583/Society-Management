...
<div class="table-container">
    <div class="table-header">Give Feedback</div>
    <div class="table-content">
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
            Enter New Feedback : <br>
            <textarea id="bigTextBox" name="feedback_content" rows="10" cols="40" required></textarea>
            <br><br>
            <input type="submit">
        </form>
        <?php
          if ($_SERVER['REQUEST_METHOD'] == 'POST') 
          {
            ...
            $qry = "update feedback_surveys set feedback='$feedback_content', date=CURRENT_TIMESTAMP where resident_id=$member_id";
            $rs = pg_query($con,$qry);

            if($rs === false || pg_affected_rows($rs) === 0) 
            {
              echo "<script> alert('Error while updating feedback into feedback_surveys table '); </script>";
              header("refresh:0;url=/Society_Management/dashboard/update_feedback_surveys.php");
            }
            
            echo '<script>alert("Success... Feedback Updated!");</script>';
