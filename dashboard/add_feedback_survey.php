    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
        ...
        <?php
          if ($_SERVER['REQUEST_METHOD'] == 'POST') 
          {
            ...
            $qry = "insert into feedback_surveys (resident_id,society_id,feedback,date) values ($member_id,$society_id,'$feedback_content',CURRENT_TIMESTAMP)";
            $rs = pg_query($con,$qry);

            if($rs === false || pg_affected_rows($rs) === 0) 
            {
              echo "<script> alert('Error while inserting feedback into feedback_surveys table '); </script>";
              header("refresh:0;url=/Society_Management/dashboard/add_feedback_survey.php");    
            }
            
            echo '<script>alert("Success... Feedback Added!");</script>';
