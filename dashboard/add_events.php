...
<div class="table-container">
    ...
    <div class="table-content">
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
            Enter Event Name : <input type="text" name="event_name" required>
            <br> <br>
            Enter Event Description : <input type="text" name="description" required>
            <br> <br>
            Enter Start Date : <input type="date" name="start_date" required>
            <br> <br>
            Enter End Date : <input type="date" name="end_date" required>         
            <br> <br>
            Enter Organizers Member Id : <input type="number" name="org_member_id" required>
            <br><br>
            <input type="submit">
        ...
        <?php
            if($_SERVER['REQUEST_METHOD'] == "POST")
            {
              ...
              $org_member_id = $_POST['org_member_id'];
              ...
              $qry = "select count(member_id) from member where member_id=$org_member_id";
              $rs = pg_query($con,$qry);
              $row = pg_fetch_row($rs);
              $count = $row[0];
              ...
              if($count == 0)
              {
                  echo "<script> alert('Member ID of Organizer does not exists!'); </script>";
                  exit(0);
              }

              $qry = "insert into events (society_id,member_id,event_name,description,start_date,end_date) values ($society_id,$org_member_id,'$event_name','$description','$start_date','$end_date')";
              $rs = pg_query($con,$qry);                
              ...
              echo '<script>alert("Success... Events Uploaded!");</script>';
              echo '<script>window.location.href = "/Society_Management/dashboard/events.php";</script>';
            }
        ?>
    </div>
  </div>
