...
<div class="table-container">
    <div class="table-header">Add New Events</div>
    <div class="table-content">
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
            Enter Complaint ID : <input type="text" name="complaint_id" required>
            <br> <br>
            <input type="radio" name="complaint_status" value="open" required> Open
            <br> <br>
            <input type="radio" name="complaint_status" value="closed" required> Closed
            <br> <br>
            <input type="radio" name="complaint_status" value="in-process" required> In-Process
            <br> <br>
            <input type="radio" name="complaint_status" value="resolved" required> Resolved
            <br> <br>
            <input type="submit">
        ...
        <?php
            if($_SERVER['REQUEST_METHOD'] == "POST")
            {
              ...
              $qry = "select count(complaint_id) from complaint where complaint_id=$complaint_id";
              $rs = pg_query($con,$qry);
              $row = pg_fetch_row($rs);
              $count = $row[0];
              ...
              if($count == 0)
              {
                echo "<script> alert('Complaint ID not Found!!!'); </script>";
                echo '<script>window.location.href = "/Society_Management/dashboard/complaints.php";</script>';
              }

              $new_complaint_status = $_POST['complaint_status'];

              $qry = "update complaint set status='$new_complaint_status' where complaint_id=$complaint_id";
              $rs = pg_query($con,$qry);
            
              if($rs === false || pg_affected_rows($rs) === 0) 
              {
                echo "<script> alert('Error while inserting event into event table '); </script>";
                header("refresh:0;url=/Society_Management/dashboard/events.php");
              }
          
              echo '<script>alert("Success... Complaint Status Changed!");</script>';
              echo '<script>window.location.href = "/Society_Management/dashboard/complaints.php";</script>';
            }
        ?>
    </div>
